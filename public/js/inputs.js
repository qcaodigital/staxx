...
$(document).on('visibilitychange', () =>
	document.visibilityState === 'visible' ? game.playSounds() : game.muteSounds()
);
...
function dropBlocksHandler(e) {
	...
	if (
		!game.state.keydown &&
		game.currentGameData.inProgress &&
		!game.state.spamPrevent &&
		!game.currentGameData.gameWon &&
		!game.currentGameData.gameOver
	) {
...
function closeModal(e) {
	if (game.state.modals.modalOpen && ([13, 27].includes(e.which) || e.type === 'click')) {
		...
		setTimeout(() => {
			!$('#generic.modal').is('.highscores, .welcome')
				? game.start()
				: $('#generic.modal').removeClass('highscores welcome');
		}, 500);
...
function closeLeaderboardModal(e) {
	if (
		game.state.modals.leaderboardModalOpen &&
		([13, 27].includes(e.which) || e.type === 'click')
	) {
...
function openLeaderboardModal(e) {
	...
	$('#leaderboard .name-list li')
		.toArray()
		.forEach((li) => $(li).html(namePlaceholder));
	$('#leaderboard .time-list li')
		.toArray()
		.forEach((li) => $(li).html(timePlaceholder));
				...
				.forEach((li, idx) => {
					setTimeout(
						() =>
							...
							$(li)
								.hide()
								.html(sorted[idx] ? sorted[idx].name : namePlaceholder)
								.fadeIn(),
						stagger * idx
					);
				});
				...
				.forEach((li, idx) => {
					setTimeout(
						() =>
							...
							$(li)
								.hide()
								.html(sorted[idx] ? `${sorted[idx].time}<span>s</span>` : '---')
								.fadeIn(),
						stagger * idx
					);
				});
...
//------------------//
//ASSISTS MODAL//
function openAssistsModal(e) {
	//Listener for assists icon
	if (Object.keys(game.state.modals).some((key) => game.state.modals[key] === true)) return;
	e.stopPropagation();
