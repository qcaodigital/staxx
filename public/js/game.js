...
class GameData {
	constructor({ numStartActiveBlocks, startingActiveBlock, speed, assists }) {
		this.dom = {
			...
			allCols: [...Array([...$('.row:first-of-type > .block')].length)].map((col, j) =>
				[...$('.block')].filter((block, k) => block.dataset.col == j)
			),
		};
		this.defaults = {
			currentSpeed: speed,
		...
		this.configs = {
			speedReductionRate: 0.8,
			activeBlockReductionRate: 0.35,
	...
	setTime(start) {
		...
		this.time.timeElapsedMS = (this.time.date.getMilliseconds() / 10)
			.toFixed(0)
			.padStart(2, '0');

		// dont allow milliseconds to display 100 due to third digit
		this.time.timeElapsedMS =
			this.time.timeElapsedMS === '100' ? '99' : this.time.timeElapsedMS;
	...
	onload() {
		...
		this.currentGameData.interval = setInterval(
			this.playNextFrame.bind(this),
			this.currentGameData.currentSpeed
		);
		...
		if (!localStorage.getItem('username')) {
			this.generateModal({
				...
				line1:
					"The objective of <strong>STAXX</strong> is to stack the blocks to the top of the grid. That's all! You only have one shot per row though so if you miss the top block, you'll have to start over. Not to mention the game will get harder as you progress.",
				line2:
					'Just press click/tap anywhere or press space bar to drop the blocks and get to the top!',
	...
	playNextFrame() {
		...
		this.dom.playRow.forEach((block, idx) => {
			...
			if (
				this.currentGameData.startingActiveBlock <= block.dataset.col &&
				block.dataset.col < this.currentGameData.lastActiveBlock
			) {
		...
		if (
			...
			this.dom.playRow.length + this.currentGameData.sideOverflow
		) {
			this.currentGameData.travelReverse = true;
		} else if (
			this.currentGameData.startingActiveBlock <=
			0 - this.currentGameData.sideOverflow
		) {
	...
	toggleAssists() {
		...
		if (this.configs.assists.highlightColumns) {
			this.dom.allCols.forEach((col, i) => {
				col.forEach((block, j) => {
					if (
						this.currentGameData.highlightedBlocks.includes(i) &&
		...
		this.dom.allRows.forEach((row, j) => {
			...
			if (targetRow === this.currentGameData.currentRow) {
				[...row.children].forEach((block, i) => {
					if (
						targetRow === 1 ||
						$(this.dom.allRows[j + 1].children[i]).hasClass('active')
					) {
	...
	dropBlocks() {
		...
		this.dom.allCols
			.filter((c, idx) => this.currentGameData.highlightedBlocks.includes(idx))
			.forEach((col, i) => {
				//Iterate through each block in columns that match the current active columns
				col.map((block, j) => {
		...
		this.currentGameData.cheatUsed = Object.keys(this.configs.assists).some(
			(key) => this.configs.assists[key] === true
		)
			? true
			: this.currentGameData.cheatUsed;
		this.validateGame();
	...
	validateGame() {
		...
		if (
			this.currentGameData.missedBlocks >=
			Math.ceil(this.currentGameData.highlightedBlocks.length)
		) {
			...
			this.currentGameData.sideOverflow =
				this.currentGameData.numStartActiveBlocks < 1
					? 0
					: this.currentGameData.sideOverflow;
			this.currentGameData.currentRow++;
			this.currentGameData.currentSpeed =
				this.currentGameData.currentSpeed * this.configs.speedReductionRate;
			this.currentGameData.lastActiveBlock =
				Math.ceil(this.currentGameData.numStartActiveBlocks) +
				this.currentGameData.startingActiveBlock;
			this.currentGameData.numStartActiveBlocks =
				this.currentGameData.numStartActiveBlocks - this.configs.activeBlockReductionRate >
				0
					? this.currentGameData.numStartActiveBlocks -
					  this.configs.activeBlockReductionRate
					: 1;
		}
	}
	...
	async gameWinAni() {
		...
		const timeString = `${(this.time.timeElapsed / -1000).toFixed(0)}.${(
			this.time.date.getMilliseconds() / 10
		)
			.toFixed(0)
			.padStart(2, '0')}`;
		...
		if (!this.currentGameData.cheatUsed) {
			const highScores =
				localStorage.getItem('high_scores') !== null
			...
			//POST win data to DB
			try {
				await axios.post('https://staxxz.herokuapp.com/scores', {
					time: +timeString,
		...
		this.currentGameData.timeElapsed =
			this.currentGameData.timeEnd - this.currentGameData.timeStart;
		this.generateModal({
			heading: 'You won!',
			line1: `Your finishing time was <strong>${timeString}</strong> seconds ${
				!leaderboardScore ? "but you didn't make it onto the leaderboard this time!" : '.'
			}`,
			line2: leaderboardScore
				? 'Congrats! You made it on the leaderboard!'
				: '<i>Try and go for a faster time!</i>',
			line3: this.currentGameData.cheatUsed
				? '<span style="font-size: .75rem">(Your score was not recorded because you had a cheat on)</span>'
				: null,
	...
	gameOverAni({ source }) {
		...
		if (source === 'miss') {
			...
			this.currentGameData.lastActiveBlocks.forEach((block, i) => {
				if (
					i >=
					this.currentGameData.lastActiveBlocks.length -
						Math.ceil(this.currentGameData.highlightedBlocks.length)
				) {
			...
			this.generateModal({
				heading: 'Yikes.',
				line1: `You lost in <strong>${(this.time.timeElapsed / -1000).toFixed(0)}.${(
					this.time.date.getMilliseconds() / 10
				)
					.toFixed(0)
					.padStart(2, '0')}</strong> seconds.`,
				line2: `${
					roundedTime >= 3
						? '<i>Better luck next time!</i>'
						: '<i>Are you even trying?</i>'
				}`,
				timeout: 150,
			});
